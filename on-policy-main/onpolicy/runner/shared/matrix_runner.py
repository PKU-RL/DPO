...
class MatrixRunner(Runner):
            ...
            if self.args.multi_rollout:
                for rollout in range(self.all_args.n_rollout_threads):
                    for step in range(self.episode_length):
                        ...
                        values, actions, action_log_probs, rnn_states, rnn_states_critic = base_ret
    ...
    def collect(self, step, rollout=None):
        ...
        ret_list = [values, actions, action_log_probs, rnn_states, rnn_states_critic]
    ...
    @torch.no_grad()
    def collect_old(self, step,rollout = None):
        self.trainer.prep_rollout()
        ...
        if self.args.use_q:
            value, action, action_log_prob, rnn_state, rnn_state_critic,target_value,target_rnn_state_critic \
                = self.trainer.policy.get_actions(share_obs_input,
                                                  ...
                                                  rnn_states_input,
                                                  rnn_states_critic_inputs,
                                                  masks_inputs,target=True,target_rnn_states_critic=target_rnn_states_critic_inputs )
            probs = self.trainer.policy.get_probs(obs_input,rnn_states_input,masks_inputs)
            baseline = value * probs
            baseline = torch.sum(baseline,dim = -1,keepdim=True)
            act_idx = action
        ...
        if self.args.use_q:
            ...
            target_values = np.array(np.split(_t2n(target_value), split_num))
            target_rnn_states_critic = np.array(np.split(_t2n(target_rnn_state_critic), split_num))
            return values, actions, action_log_probs, rnn_states, rnn_states_critic,target_values,target_rnn_states_critic,baselines,old_probs_all,old_values_all
        else:
            return values, actions, action_log_probs, rnn_states, rnn_states_critic

    def insert_old(self, data,rollout= None):
        if self.args.use_q:
            obs, share_obs, rewards, dones, infos, available_actions, \
            values, actions, action_log_probs, rnn_states, rnn_states_critic,target_rnn_states_critic,value_curr,baseline,old_probs_all,old_values_all = data
        else:
            obs, share_obs, rewards, dones, infos, available_actions, \
            values, actions, action_log_probs, rnn_states, rnn_states_critic = data
            target_rnn_states_critic, value_curr, baseline,old_probs_all,old_values_all = None,None,None,None,None
        ...
        if self.args.use_q:
            target_rnn_states_critic[dones_env == True] = np.zeros(
                ((dones_env == True).sum(), self.num_agents, *self.buffer.target_rnn_states_critic.shape[3:]), dtype=np.float32)
        if rollout is None:
            masks = np.ones((self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
            active_masks = np.ones((self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
        else:

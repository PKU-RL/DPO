...
class MujocoRunner(Runner):
            ...
            if self.args.multi_rollout:
                for rollout in range(self.all_args.n_rollout_threads):
                    for step in range(self.episode_length):
                        # Sample actions
                        base_ret,q_ret,sp_ret = self.collect(step, rollout)
                ...
                for step in range(self.episode_length):
                    ...
                    base_ret, q_ret, sp_ret = self.collect(step)
                    values, actions, action_log_probs, rnn_states, rnn_states_critic = base_ret
    ...
    def collect(self, step,rollout = None):
            ...
            for i in range(self.args.sp_num):
                value, action, action_log_prob, rnn_state, rnn_state_critic \
                    = self.trainer.policy.get_actions(share_obs_input,
                                                      ...
                                                      masks_inputs)
                if self.args.sp_check:
                    print('sp {}: value = {}, action = {}  action_log_prob = {}'.format(i,value,action,action_log_prob))
        ...
        return ret_list,q_ret_list,sp_ret_list

    def insert(self, data,rollout= None):
        ret_list, q_ret_list, sp_ret_list = data
        ...
        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic,
                           ...
                           sp_value = sp_value,sp_prob = sp_prob)



    def log_train(self, train_infos, total_num_steps):
        # print('shape = {}'.format(np.shape(self.buffer.rewards[self.buffer.step])))
        if self.all_args.env_name in ['matrix_game','mujoco']:
